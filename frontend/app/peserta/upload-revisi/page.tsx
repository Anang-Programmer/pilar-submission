...
export default function UploadRevisiPage() {
  ...
  return <><div className={styles.title}><div><h1>Upload Revisi</h1><p>Kirim versi artikel setelah menindaklanjuti catatan reviewer.</p></div></div>{error && <div className={styles.error}>{error}</div>}{message && <div className={styles.notice}>{message}</div>}<div className={styles.panel}>{articles.length === 0 ? <div className={styles.empty}>Belum ada artikel.</div> : <>
    ...
    {!article ? <><div className={styles.empty}>Belum ada artikel yang memerlukan revisi.</div><div className={styles.actionRow}><button className={`${styles.btn} ${styles.primary}`} disabled>Belum Ada Revisi</button></div></> : <><div className={styles.field} style={{ marginTop: 18 }}><label>File Artikel Revisi</label><div className={styles.fileBox}><input type="file" accept=".pdf,.doc,.docx" onChange={(e) => setFile(e.target.files?.[0] || null)} /><div className={styles.fileName}>{file ? `${file.name} · ${Math.round(file.size / 1024)} KB` : "PDF/DOC/DOCX · Maks. 10 MB"}</div></div></div><div className={styles.field}><label>Ringkasan Perbaikan</label><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Contoh: memperbaiki pendahuluan, menambahkan tabel hasil pengujian, dan memperjelas metode penelitian." /></div><div className={styles.actionRow}><button className={`${styles.btn} ${styles.primary}`} onClick={submit} disabled={saving || !hasRevision}>{saving ? "Mengirim..." : hasRevision ? "Kirim Revisi" : "Belum Ada Revisi"}</button></div></>}</>}</div></>;
}
