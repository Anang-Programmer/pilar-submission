...
function displayStatus(item: any): string {
  ...
  if (["terpilih", "selected", "select"].includes(projectStatus) || ["terpilih", "selected", "select"].includes(selectionStatus)) {
    return "Terpilih";
  }
  return "Belum Terpilih";
}

export default function DashboardUserProjectsPage() {
  ...
  useEffect(() => {
    let active = true;
    api.admin.projects()
      .then((result) => { if (active) setProjects(Array.isArray(result) ? result : []); })
      .catch((e) => { if (active) setError(e instanceof Error ? e.message : "Gagal memuat proyek"); })
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <div className={styles.actions}>
                      <button type="button" className={`${styles.btn} ${styles.small} ${styles.secondary}`} onClick={() => handleArticleAction(item.id, "view")} disabled={actionLoading === item.id || !item.article?.current_version?.has_file}>Lihat</button>
                      <button type="button" className={`${styles.btn} ${styles.small} ${styles.primary}`} onClick={() => handleArticleAction(item.id, "download")} disabled={actionLoading === item.id || !item.article?.current_version?.has_file}>Download</button>
                    </div>
                  </td>
                </tr>
              ))}
