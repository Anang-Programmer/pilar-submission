...
export default function DashboardUserParticipantsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [p, pr, a, m] = await Promise.allSettled([api.admin.participants(), api.admin.projects(), api.admin.articlesAdmin(), api.admin.mentorshipAssignments()]);
      const errors: string[] = [];
  ...
  const rows = useMemo(() => {
    ...
    return participants.filter((user) => {
      ...
      return !q || `${student.full_name || ""} ${student.nim || ""} ${user.email || ""}`.toLowerCase().includes(q);
    }).map((user) => {
      const student = user.student_profile || {};
